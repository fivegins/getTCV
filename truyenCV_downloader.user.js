...
(function ($, window, document) {
    ...
    function cleanHtml(str) {
    	...
    	str = str.replace(/-(?:\s*-)+/gm, '');
    	str = str.replace(/_(?:\s*_)+/gm, '');
    	str = str.replace(/,(?:\s*,)+/gm, '');
    	str = str.replace(/"\.\.\."/gm, '"Lặng!"');
    	str = str.replace(/\?\s*!/gm, '?');
    	str = str.replace(/ ah/gmi, ' a');
    	str = str.replace(/ ria/gmi, ' dia');
    	str = str.replace(/BOANG/gm, 'Boang');
    	str = str.replace(/~/gm, '-');
    	str = str.replace(/\s*cvt.*?</gmi, '<');
    	str = str.replace(/:\s*\.\.\./gmi, ': "Lặng!"');
    ...
    function cleanTitle(str) {
    	str = str.replace(/\s*Chương\s*0(\d+)/i, 'Chương $1');
    	str = str.replace(/:+/, ':');
    	str = str.replace(/\s*【.*/, '');
    	return str;
    }
